...
import { FederatedOIDCUserInfo, LegacyUserProfile } from "../models/principal";
...
class AuthenticateUserService extends IdentityService {
    ...
    public async handleFederatedOIDCProviderReturn(state: string, code: string): Promise<FederatedOidcProviderReturnResponse> {
        if(state.startsWith(FEDERATED_AUTH_TEST_STATE_PARAM_PREFIX)){
            return this.handleFederatedAuthTest(state, code);
        }
        else{
            return this.handleFederatedAuth(state, code);
        } 
    }

    protected async handleFederatedAuth(state: string, code: string): Promise<FederatedOidcProviderReturnResponse> {
        const federatedOidcProviderReturnResponse: FederatedOidcProviderReturnResponse = {
        ...
        if(authRel === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"Federated authorization failed. The authorization state was invalid or was not found."}`;            
            return federatedOidcProviderReturnResponse;
        }
       
    
        if(authRel.expiresAtMs < Date.now()){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"Federated authorization failed. The authorization state has expired."}`;            
            return federatedOidcProviderReturnResponse;
        }
        
        const tenant: Tenant | null = await tenantDao.getTenantById(authRel.initTenantId);
        if(tenant === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"No valid tenant was found for authentication."}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        if(oidcProvider === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"Federated authorization failed. No Federated OIDC Provider was found."}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        if(!wellKnownConfig){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The OIDC provider does not have a well-known URI"}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        if(tokenResponse === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The authorization code or federated OIDC provider was invalid."}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        if(userInfo === null || userInfo.email === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The user information from the federated OIDC provider cannot be obtained or insufficient scope for retrieving email was provided."}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        if(userByFederatedSubjectId !== null && userByEmail !== null && userByFederatedSubjectId.userId !== userByEmail.userId){
            ...
            await this.updateUserTenantRel(tenant, userByFederatedSubjectId);
        ...
        // Otherwise, if there are no existing records, then create a new user based on the userinfo retrieved from the 3rd party IdP
            ...
            const newUser: User = this.userInfoToUser(userInfo);
            if(userByPhone === null){
            ...
            await this.updateUserTenantRel(tenant, newUser);
        ...
        if(canonicalUser){
            ...
            if(authRel.federatedOIDCAuthorizationRelType === FederatedOidcAuthorizationRelType.AuthorizationRelTypeClientAuth){
                await this.saveAuthorizationCodeData(authorizationCode, authRel, canonicalUser);
                federatedOidcProviderReturnResponse.authenticationState = AuthenticationState.RedirectBackToApplication;
                federatedOidcProviderReturnResponse.redirectUri = `${authRel.initRedirectUri}?code=${authorizationCode}&state=${authRel.initState}`;
            }
            else{
                ...
                federatedOidcProviderReturnResponse.authenticationState = AuthenticationState.RedirectToIamPortal;
                federatedOidcProviderReturnResponse.redirectUri = `/${tenant.tenantId}`;
                federatedOidcProviderReturnResponse.accessToken = result?.accessToken;
                federatedOidcProviderReturnResponse.tokenExpiresAtMs = Date.now() + (ttlSeconds * 1000);
            }
            return federatedOidcProviderReturnResponse
        }
    ...
    protected async updateUserTenantRel(tenant: Tenant, user: User): Promise<void> {
    ...
    protected async saveAuthorizationCodeData(authorizationCode: string, authRel: FederatedOidcAuthorizationRel, user: User): Promise<AuthorizationCodeData>{
    ...
    protected userInfoToUser(userInfo: FederatedOIDCUserInfo): User {
        let langCode = "en";
        let countryCode = "US";
        if(userInfo.locale){
    ...
    protected async handleFederatedAuthTest(state: string, code: string): Promise<FederatedOidcProviderReturnResponse> {
        const federatedOidcProviderReturnResponse: FederatedOidcProviderReturnResponse = {
            authenticationState: AuthenticationState.Error,
            redirectUri: "",
            accessToken: null,
        ...
        if(federatedAuthTest === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The token used for testing the OIDC provider is invalid or has expired"}`;
            return federatedOidcProviderReturnResponse;
        }
        if(federatedAuthTest.expiresAtMs < Date.now()){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The token used for testing the OIDC provider has expired"}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        if(tokenResponse === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The authorization code or client configuration used for testing the OIDC provider is invalid"}`;
            return federatedOidcProviderReturnResponse;
        }
    
        const userInfo: FederatedOIDCUserInfo | null = await oidcServiceUtils.getFederatedOIDCUserInfo(wellKnownConfig.userinfo_endpoint, tokenResponse.access_token);
        if(userInfo === null || userInfo.email === null){
            federatedOidcProviderReturnResponse.redirectUri = `/access-error?access_error_code=00076&extended_message=${"The user information for testing the OIDC provider cannot be obtained."}`;
            return federatedOidcProviderReturnResponse;
        }
        ...
        federatedOidcProviderReturnResponse.authenticationState = AuthenticationState.RedirectToIamPortal;        
        federatedOidcProviderReturnResponse.redirectUri = "/authorize/auth-successful";        
        return federatedOidcProviderReturnResponse;
    }
